...
from agent.brief_generator.bench import bench_capacity_block as _bench_block
...
- Identify which top-quartile practices the prospect lacks
...
ICP SEGMENTS (apply in priority order — Segment 2 dominates when multiple match):
- Segment 2: 200–2000 headcount, layoff in last 120 days, ≥3 open eng roles post-layoff.
...
- Set icp_segment="ambiguous" if confidence < 0.6 or no segment criteria clearly fire.
Disqualifiers (set icp_segment="disqualified"): >5000 employees, <$2M seed-only funding,
consumer apps, anti-offshore founder stance.
...
- Never use clichés: "top talent", "world-class", "rockstar", "ninja", "bench" (use "engineering capacity").
...
{{
  "hiring_signal_brief": {{
    ...
    "confidence": <0.0-1.0>,
    "icp_segment": "<Segment 1|2|3|4|ambiguous|disqualified>",
